...
UI레이어는 Jetpack Compose를 사용해 구성하였으며, 화면에 필요한 데이터는 ViewModel에서 StateFlow를 통해 관리합니다

ViewModel은 앱의 상태를 관리하고, 비즈니스 로직을 처리하는 역할을 수행하며, UI는 이 상태를 관찰하여 데이터가 변경될 때 자동으로 화면이 갱신되도록 구성하였습니다

데이터 레이어는 Repository를 통해 구성되며, Firebase(Firestore, Authentication)와 같은 외부 데이터 소스와의 통신을 담당합니다. 이를 통해 데이터 접근 로직을 분리하고 유지보수성을 향상시켰습니다

객체 간의 결합도를 낮추기 위해 AppContainer를 사용하여 필요한 객체들을 한번만 생성하고 필요한 곳에서 사용하는 DI(Dependency Injection)  구조를 적용하여 각 계층에서 재사용할 수 있도록 설계하였습니다

전체 구조는 UI -> ViewModel -> Repository -> DataSource 로 이어지는 데이터 흐름을 따르며 이를 통해 구조의 일관성과 안정성을 확보하였습니다
